  ...
  <div class="lottery-ball-selector">
    <div class="selector-container">
      ...
      <section>
        <div class="ball-list">
          <span
            v-for="(ball, index) in selfReviewSelectedList"
            :key="index"
            class="ball-item"
            :class="{ selected: true }"
...
const selfReviewSelectedList = ref<BallOption[]>(props.reviewSelectedList ?? [])
...
function emitSelectionChange(): void {
  emit(
    'selectionChanged',
    selfReviewSelectedList.value.filter((ball) => ball.selected)
  )
}
...
function toggleBallSelection(ball: BallOption): void {
  ...
  if (ball.selected) {
    ball.selected = false
    const index = selfReviewSelectedList.value.findIndex((b) => b.value === ball.value)
    if (index !== -1) {
      selfReviewSelectedList.value.splice(index, 1)
    }
    emitSelectionChange()
    return
  }
  ...
  ball.selected = true
  selfReviewSelectedList.value.push(ball)
  emitSelectionChange()
}
