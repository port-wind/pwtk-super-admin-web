...
export function useLotteryParticipationLogic(state: Reactive<ICreateBbsPostQueryParams | IUpdateBbsPostQueryParams>) {
  ...
  const selectedMainPlayTypeCodes = ref<string[]>([])
  ...
  const setPredictionsByValue = (predictions: Array<PredictionInfo>) => {
    state.predictions = predictions
  }
  function getLotteryPredictionsFrom(
    rules: Array<PlayRuleExtended>,
    r: PlayRuleExtended,
    selectedBalls: BallOption[]
  ): Array<PredictionInfo> {
    return rules.map((rule) => {
      if (rule.code !== r.code) {
        return state.predictions!.find((p) => p.playTypeCode === rule.code)!
      }
      const predict = selectedBalls.map((ball) => ball.value)
      return {
        gameType: state.postGametypeRef!,
        playTypeCode: rule.code,
        name: rule.name,
  ...
  const getPredictionModeByRule = (ruleCode: string): PostPredictionRuleExtended => {
    return postPredictionRules.value.find((r1) => !!r1.playRules.find((r2) => r2.code === ruleCode))!
  }
  const getReviewSelectedListBy = (
    rule: PlayRuleExtended,
    reviewedSelected: Array<{ code: string; ballList: BallOption[] }>
  ): BallOption[] => {
    const { ballList } = reviewedSelected.length > 0 ? reviewedSelected[0] : { ballList: [] }
    return ballList.map((ball) => ({ value: ball.value, selected: true }))
  }
