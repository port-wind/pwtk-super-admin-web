    ...
    <mTabPane label="彩种新增">
      ...
      >
        <el-form-item label="彩种ID:" prop="gameType">
          <el-input v-model="lineFormData.gameType" :maxlength="128" />
        </el-form-item>
...
import { gameTypePlatformCreate, gameTypePlatformListAll } from '@/api/system/gameTypeManagement'
...
const handleSubmit = async () => {
  formRef.value.validate(async (valid: boolean) => {
    if (valid) {
      ...
      try {
        const response = (await gameTypePlatformCreate(params)) as any
        if (response.success) {
...
onMounted(async () => {
  try {
    ...
    console.log(res)
    if (res.success) {
      Object.assign(allDataExisted, res.data)
    }
  } catch (error) {
