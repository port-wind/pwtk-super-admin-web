...
# 设置环境变量，允许跳过 pre-push hook 中的分支保护检查
...
# Step 1: Check if we are on main branch
info_message "Checking current branch..."
CURRENT_BRANCH=$(git branch --show-current)
if [ "$CURRENT_BRANCH" != "main" ]; then
    error_exit "Must be on 'main' branch. Current branch: $CURRENT_BRANCH. Please switch to main branch first: git checkout main"
fi
success_message "Currently on main branch"
...
# Step 4: Check if local main is behind remote
info_message "Checking if local main branch is up to date..."
LOCAL_COMMIT=$(git rev-parse main)
REMOTE_COMMIT=$(git rev-parse origin/main)

if [ "$LOCAL_COMMIT" != "$REMOTE_COMMIT" ]; then
    info_message "Local main branch is behind remote. Pulling latest changes..."
    git pull origin main || error_exit "Failed to pull latest changes from origin/main"
    success_message "Successfully pulled latest changes"
else
    success_message "Local main branch is up to date"
fi
